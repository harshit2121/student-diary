...
export default function Attendance() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      // Optionally fetch users/{uid} to get teacher name
      try {
  ...
  // Load distinct class/section options from students
  useEffect(() => {
    const run = async () => {
  ...
  const setStatus = (rn, status) => rn && setMarks((p) => ({ ...p, [rn]: { ...(p[rn]||{}), status } }));
  const setNote = (rn, note) => rn && setMarks((p) => ({ ...p, [rn]: { ...(p[rn]||{}), note } }));
  const markAll = (status) => {
    const next = {};
    students.forEach((s) => { if (s.rollNumber) next[s.rollNumber] = { ...(marks[s.rollNumber]||{}), status }; });
    setMarks(next);
  };
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="mt-4 rounded-xl bg-white border border-slate-200 p-4 shadow-sm">
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
              <select value={grade} onChange={(e)=>setGrade(e.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm">
                <option value="">Class</option>
                {grades.map((g)=> <option key={g} value={g}>{g}</option>)}
              </select>
              <select value={section} onChange={(e)=>setSection(e.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm">
                <option value="">Section</option>
                {sections.map((s)=> <option key={s} value={s}>{s}</option>)}
              </select>
              <input type="date" value={date} onChange={(e)=>setDate(e.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm" />
              <motion.button whileHover={{scale:1.01}} whileTap={{scale:0.98}} onClick={loadStudents} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm hover:bg-slate-50">
                {loading ? "Loading…" : "Load Students"}
              </motion.button>
              <div className="flex items-center gap-2">
                <button onClick={()=>markAll("Present")} className="rounded-md bg-emerald-600 text-white px-3 py-2 text-sm hover:bg-emerald-500 w-full">All Present</button>
                <button onClick={()=>markAll("Absent")} className="rounded-md bg-rose-600 text-white px-3 py-2 text-sm hover:bg-rose-500 w-full">All Absent</button>
              </div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <div className="rounded-xl bg-white border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 sticky top-0 bg-white/95 backdrop-blur z-10">
            ...
            <button onClick={saveAll} disabled={saving} className="rounded-md bg-indigo-600 text-white px-4 py-2 text-sm hover:bg-indigo-500 disabled:opacity-60">
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
              ...
              return (
                <div key={s.id} className="px-4 py-3 flex items-center gap-3">
                    ...
                    <button
                      ...
                      className={
                        (sel === "Absent"
                          ...
                          : "bg-white text-rose-700 border-rose-600 hover:bg-rose-50") +
                        " px-3 py-1 rounded-md border text-sm transition-colors"
                      }
                    >
                      Absent
                  ...
                  <input
                    ...
                    className="ml-3 w-56 rounded-md border border-slate-300 px-2 py-1 text-sm"
                  />
                </div>
              );
            })}
